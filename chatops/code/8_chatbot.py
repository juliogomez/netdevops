    ...
    if bot_id == data.get('data').get('personId'):
        ...
        if message_text.startswith('/meraki'):
            ...
            if action == 'networks':
                ...
                first_network = network_list[0].get('id')
                ssid_list = dashboard.wireless.getNetworkWirelessSsids(first_network)
                reply = f'Your first network with ID {first_network} includes the following SSIDs:'
                for ssid in ssid_list:
                    reply += f"\n-SSID {ssid['name']} with number {ssid['number']} is enabled: {ssid['enabled']}"
        else:
